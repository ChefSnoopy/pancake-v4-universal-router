...
library CalldataDecoder {
    ...
    /// @dev equivalent to: abi.decode(params, (bool, bytes)) in calldata
    function decodeBoolAndBytes(bytes calldata _bytes)
        internal
        pure
        returns (bool isDefault, bytes calldata extraData)
    {
        assembly ("memory-safe") {
            // Read the `bool` value
            isDefault := calldataload(_bytes.offset)

            let extraPtr := add(_bytes.offset, calldataload(add(_bytes.offset, 0x20)))

            extraData.length := calldataload(extraPtr)
            extraData.offset := add(extraPtr, 0x20)
        }
    }
